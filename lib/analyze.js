...
/**
 * Deep hiring-manager review. Throws on failure so the caller can fall back to
 * the rules-only report; the shared client handles timeout, retry and salvage.
 */
export async function deepAnalyze({ text, jobId, seniority, jobDescription, rules }) {
  return generateJson({
    prompt: buildPrompt({ text, jobId, seniority, jobDescription, rules }),
