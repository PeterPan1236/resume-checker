...
function findContact(text) {
  ...
  const site =
    ...
    null;
  // Only the header carries contact details. Scanning the whole document made any
  // "Title, Company" line ("Senior Software Engineer, Northwind") register as a
  // location, so a resume with no location still scored as though it had one.
  const lines = text.split('\n');
  const firstHeading = lines.findIndex((l) => SECTION_HEAD_RE.test(l.trim()));
  const header = lines.slice(0, firstHeading === -1 ? 6 : Math.min(firstHeading, 8)).join('\n');
  const location =
...
export function runRules(text, { jobId, seniority, jobDescription, pages, fileKind }) {
  ...
  if (!contact.location) {
    add('medium', 'Contact', 'No city or region in the contact block.', 'Add "City, Country" (or "City, ST") to the header. Recruiters filter on location, and a blank one reads as either relocating or hiding something.');
  }
  ...
  // --- Dates ---
  if (dates.ranges < 2) {
    add('high', 'Dates', `Only ${plural(dates.ranges, 'date range', 'date ranges')} detected across the resume.`, 'Give every role and degree a "Mon YYYY – Mon YYYY" range. Missing dates read as a hidden gap.');
  }
  if (dates.ranges >= 1 && !dates.hasPresent) {
    add('medium', 'Dates', 'No role is marked as current — every date range has a closed end date.', 'End your current role with "Present". Otherwise the latest end date reads as the date you left, and the resume looks stale.');
  }
  ...
  if (bullets.length === 0) {
    ...
    if (longBullets.length > 0) {
      add('medium', 'Language', `${plural(longBullets.length, 'bullet runs', 'bullets run')} longer than 32 words.`, 'Cap bullets at ~2 lines. Split or cut the setup and keep the outcome.');
    }
  ...
  if (hardPct < 25) {
    add('high', 'Job fit', `Only ${kw.hardHit.length} of ${kw.profile.hardSkills.length} terms common to ${kw.profile.label} resumes appear.`, `Weave in the tools you actually use from: ${kw.hardMiss.slice(0, 10).join(', ')}.`);
  }
