...
import { checkLock, recordFailure, clearFailures, listLockouts, MAX_FAILURES } from './rate-limit.js';
...
export async function handleAdmin({ db, method, url, authorization, adminToken, ip }) {
  // Lockout is checked before the token, so a locked-out address learns nothing
  // from the response about whether its guess was right.
  const lock = await checkLock(db, ip);
  if (lock.locked) {
    return {
      status: 429,
      headers: { 'retry-after': String(lock.retryAfter) },
      body: {
        error: `Too many failed attempts. Try again in ${Math.ceil(lock.retryAfter / 60)} minute(s).`,
  ...
  if (!auth.ok) {
    // A missing ADMIN_TOKEN (503) is a server misconfiguration, not a guess, so
    // only real credential failures are counted.
    if (auth.status === 401) {
      const state = await recordFailure(db, ip);
      const body = state.locked
        ? {
            error: `Too many failed attempts. Try again in ${Math.ceil(state.retryAfter / 60)} minute(s).`,
            retryAfter: state.retryAfter
          }
        : { error: auth.error, attemptsRemaining: state.remaining };
      return {
        status: state.locked ? 429 : 401,
        headers: state.locked ? { 'retry-after': String(state.retryAfter) } : undefined,
        body
      };
    }
  ...
  if (pathname === '/api/admin/lockouts' && method === 'GET') {
    return { status: 200, body: { maxFailures: MAX_FAILURES, lockouts: await listLockouts(db) } };
  }
